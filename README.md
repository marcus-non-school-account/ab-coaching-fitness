...
1. **The enquiry form does not send anything yet.** `get-started.html` has a
   `YOUR_FORM_ID` placeholder. Augusto signs up free at
   [formspree.io](https://formspree.io), creates a form, and pastes the endpoint
   ID in. Vercel has no built-in form handling, so this step is required — until
   it is done, the site's only call to action is a dead end.
2. **Have Augusto read it.** The biography on `/about` is drawn from his own
   public Instagram post, and "3 coaching slots open this month" appears twice
   and will go stale.
